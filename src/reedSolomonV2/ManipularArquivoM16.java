 ...
 * 	TODO: Percorrer o vetor de dados e guardar a redundancia
	...
	TODO: Rotina de codificacao/decodificacao para n iteracoes com n = tamanhoArquivo/k_MAX
 */

public class ManipularArquivoM16 {

	public static void main(String[] args) throws IOException, ReedSolomonException {

		// n=2501; k=2301; n-k=200; t=100
		String localAbsoluto = "Z:\\@desenvolvimento\\workspace\\Testes-com-RS-GF(2^16)\\GPEs em atividade.pdf";
		ManipularArquivoM16.degradacaoCorretiva(localAbsoluto);
	}
	...
	private static void degradacaoCorretiva(String localAbsoluto) throws IOException, ReedSolomonException {
		//Tamanho maximo para degradacao de um arquivo
		...
		int[] arquivo = ManipularArquivoM16.byteSignedParaUnsigned(localAbsoluto, qtdSimbolosParidade);
		...
		int[] vetorParidade = ManipularArquivoM16.salvaVetorRedundancia(arquivo, qtdSimbolosParidade);

		// Apaga redundancia
		ManipularArquivoM16.zerarRedundancia(arquivo, qtdSimbolosParidade);

		// Transformar de int[] para byte[]
		byte[] codificado = ManipularArquivoM16.byteUnsignedParaSigned(arquivo);

		// Guardar a redundancia em um arquivo sem converter de int[] para byte[]
		ManipularArquivoM16.gravarRedundancia(vetorParidade, localAbsoluto);

		// Cria vetor de bytes ja codificados pelo encoder e corrompido t posicoes
		ManipularArquivoM16.corrompeDado(codificado, t);

		// Gravar arquivo codificado e corrompido
		ManipularArquivoM16.gravaArquivoCodificado(codificado, localAbsoluto);

		// Transformar de byte[] para int[] guardar arquivo corrompido
		int[] voltaDeByte = ManipularArquivoM16.byteSignedParaUnsignedSemParidade(localAbsoluto);
		...
		// indices de paridade
		byte[] decodificado = ManipularArquivoM16.byteUnsignedParaSigned(voltaDeByte);
		ManipularArquivoM16.gravaArquivoDecodificado(decodificado, localAbsoluto);
	}
